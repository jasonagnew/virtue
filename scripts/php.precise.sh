...
if [ -z "$PHP_VERISON" ]; then
    php_version="distributed"
else
    php_version="$PHP_VERISON"
fi

echo ">>> Installing PHP $PHP_VERISON version"
...
# PHP Date Timezone
sed -i "s/;date.timezone =.*/date.timezone = ${SERVER_TIMEZONE/\//\\/}/" /etc/php5/fpm/php.ini
sed -i "s/;date.timezone =.*/date.timezone = ${SERVER_TIMEZONE/\//\\/}/" /etc/php5/cli/php.ini
